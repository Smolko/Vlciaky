 ...
 * @property integer $export_import_type
 ...
 */
class Dog extends BaseModel{  //CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('export_import_type', 'required'),
            array('sex, tattoo,export_import_type, id_fertilisation, id_owner, id_old_owner, id_kennel, dlk, dbk, dwarf, dm, state', 'numerical', 'integerOnly' => true),
            ...
            array('id, name, sex, color, birthday, deathday, death_cause, breed, old_regnumber, new_regnumber, '
                . 'tattoo, chip, export_import, export_import_type, breeding,  id_fertilisation, id_owner, id_old_owner,id_kennel, dlk, dbk, dwarf, dm, dna, dlk_vet, dlk_date, dbk_vet, dbk_date, dwarf_vet, dwarf_date, dm_vet, dm_date, created_at, updated_at, state', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function search() {
        ...
        $criteria->compare('export_import_type', $this->export_import_type);
    ...
    public function getMaleCount() {
        return count($this->findAllByAttributes(array('sex' => self::MALE)));
    }

    public function getFemaleCount() {
        return count($this->findAll($this->findAllByAttributes(array('sex' => self::FEMALE))));
    }

    public function getMaleList() {
        return CHtml::listData($this->findAllByAttributes(array('sex' => self::MALE)), 'id', 'name');
    }

    public function getFemaleList() {
        return CHtml::listData($this->findAllByAttributes(array('sex' => self::FEMALE)), 'id', 'name');
    }
    ...
    public function getExportDogCount() {
        return count($this->findAll(array('condition' => 'export_import_type=' . self::DOG_EXPORT)));
    }

    public function getExportDogCountYear($year) {
        return count($this->findAll(array('condition' => 'export_import_type=' . self::DOG_EXPORT . ' AND ' . "export_import BETWEEN '" . date($year . '-01-01') . "' AND '" . date($year . '-12-31') . "'")));
    }

    public function getImportDogCount() {
        return count($this->findAll(array('condition' => 'export_import_type=' . self::DOG_IMPORT)));
    }

    public function getImportDogCountYear($year) {
        return count($this->findAll(array('condition' => 'export_import_type=' . self::DOG_IMPORT . ' AND ' . "export_import BETWEEN '" . date($year . '-01-01') . "' AND '" . date($year . '-12-31') . "'")));
    }

    public function getDLKStatistisc($year) {

        $criteria = new CDbCriteria;
        $criteria->select = 'dlk, count(id) as id';
        $criteria->condition = "dlk_date BETWEEN '" . date($year . '-01-01') . "' AND '" . date($year . '-12-31') . "'";
        $criteria->group = 'dlk';

//                var_dump($this->findAll($criteria));
//                exit;
        return $this->findAll($criteria);
    }

    public function getDBKStatistisc($year) {

        $criteria = new CDbCriteria;
        $criteria->select = 'dbk, count(id) AS id';
        $criteria->condition = "dbk_date BETWEEN '" . date($year . '-01-01') . "' AND '" . date($year . '-12-31') . "'";
        $criteria->group = 'dbk';
    ...
    public function getDLKSCounts($year) {

        $criteria = new CDbCriteria;
        $criteria->select = 'sex, count(id) as id';
        $criteria->condition = "dlk_date BETWEEN '" . date($year . '-01-01') . "' AND '" . date($year . '-12-31') . "'";
        $criteria->group = 'sex';
    ...
    public function getDBKSCounts($year) {
